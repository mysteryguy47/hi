...
class LoginRequest(BaseModel):
    credential: str | None = None  # Google OAuth token (primary field)
    token: str | None = None  # Alternative field name (for compatibility)
    
    @model_validator(mode='after')
    def validate_token_present(self):
        """Ensure at least one token field is provided."""
        if not self.credential and not self.token:
            raise ValueError("Either 'credential' or 'token' must be provided")
        if self.credential and not self.credential.strip():
    ...
    def get_token(self) -> str:
        """Get the token from either credential or token field."""
        if self.credential:
