...
export default function PaperAttempt() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitPaperAttempt(activeAttemptId, answers, timeTaken);
          ...
          <div className="bg-slate-800 dark:bg-slate-800 rounded-2xl shadow-xl dark:shadow-2xl p-8 mb-6 border border-slate-700 dark:border-slate-700 transition-all duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-yellow-900/30 dark:bg-yellow-900/30 border border-yellow-700 dark:border-yellow-700 rounded-xl p-4 text-center transition-all duration-300">
                <div className="text-3xl font-bold text-yellow-400 dark:text-yellow-400">{result.points_earned}</div>
            ...
            <div className="space-y-4">
              {generatedBlocks.map((block, blockIdx) => (
                <div key={blockIdx} className="mb-6">
                  {block.config.title && (
                    <h3 className="text-lg font-semibold text-slate-300 dark:text-slate-300 mb-4">{block.config.title}</h3>
                  )}
                      ...
                      return (
                        <div
                          ...
                          className={`p-4 rounded-lg border-2 transition-all duration-300 ${
                            isCorrect
                              ? "bg-green-900/30 dark:bg-green-900/30 border-green-700 dark:border-green-700"
                              : "bg-red-900/30 dark:bg-red-900/30 border-red-700 dark:border-red-700"
                          }`}
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-sm">
                                <span className={isCorrect ? "text-green-400 dark:text-green-400" : "text-red-400 dark:text-red-400"}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 py-8">
      <div className="container mx-auto px-4 max-w-6xl">
        {/* Header */}
        <div className="bg-slate-800 dark:bg-slate-800 rounded-2xl shadow-xl dark:shadow-2xl p-6 mb-6 sticky top-4 z-10 border border-slate-700 dark:border-slate-700 transition-all duration-300">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              <button
                onClick={() => setShowExitConfirm(true)}
                className="p-2 hover:bg-slate-700 dark:hover:bg-slate-700 rounded-lg transition-colors" 
                title="Exit Paper"
              >
                <ArrowLeft className="w-5 h-5 text-slate-300 dark:text-slate-300" />
              </button>
              <div>
                <h1 className="text-2xl font-bold text-white dark:text-white">{paperConfig.title}</h1>
                <p className="text-slate-300 dark:text-slate-300 text-sm mt-1">{paperConfig.level}</p>
              </div>
            </div>
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                <Clock className="w-5 h-5 text-slate-300 dark:text-slate-300" />
                <span className="text-lg font-semibold text-white dark:text-white">{formatTime(currentTime)}</span>
              </div>
