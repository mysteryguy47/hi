...
def render_vertical_question(question, show_answer: bool = False) -> str:
    ...
    # PRIMARY CHECK: text field contains decimal points (most reliable - backend always sets this for decimal_add_sub)
    is_decimal = False
    if hasattr(question, 'text') and question.text and '.' in str(question.text):
        is_decimal = True
    # FALLBACK: Check operator and operands format
    elif hasattr(question, 'operator') and question.operator:
        operator_str = str(question.operator)
        if operator_str == "±" or operator_str == "\u00B1":
            # Check operands format - for decimal_add_sub: operands are 10-9999, NOT multiples of 10
            if hasattr(question, 'operands') and question.operands:
                is_decimal = all(op >= 10 and op <= 9999 and op % 10 != 0 for op in question.operands)
            ...
            if has_vertical:
                ...
                for chunk_start in range(0, len(vertical_questions), 10):
                    ...
                    for q in chunk:
                        ...
                        if is_block_decimal:
                            is_decimal = True
                        # SECONDARY CHECK: text field contains decimal points
                        elif hasattr(q, 'text') and q.text:
                            text_val = str(q.text)
                            if '.' in text_val:
                                is_decimal = True
                        # FALLBACK 1: Check operator
                        if not is_decimal and hasattr(q, 'operator') and q.operator:
                            operator_str = str(q.operator)
                            if operator_str == "±" or operator_str == "\u00B1":
                                is_decimal = True
                        # FALLBACK 2: Check operands format
                        if not is_decimal and hasattr(q, 'operands') and q.operands:
                            # For decimal_add_sub: operands are 10-9999, NOT multiples of 10
                            if all(op >= 10 and op <= 9999 and op % 10 != 0 for op in q.operands):
                                is_decimal = True
